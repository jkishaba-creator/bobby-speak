...
(function (root) {
  ...
  function applySpokenPunctuation(text) {
    let out = text;
    const literalSpokenPunctuation = [];
    out = out.replace(
      ...
      (match) => {
        const token = "__BOBBY_SPEAK_LITERAL_PUNCTUATION_" + literalSpokenPunctuation.length + "__";
        literalSpokenPunctuation.push(match);
        return token;
      }
    );
    for (const [re, sub] of SPOKEN) out = out.replace(re, sub);
    literalSpokenPunctuation.forEach((literal, index) => {
      out = out.replace("__BOBBY_SPEAK_LITERAL_PUNCTUATION_" + index + "__", literal);
    });
    return out;
  }
